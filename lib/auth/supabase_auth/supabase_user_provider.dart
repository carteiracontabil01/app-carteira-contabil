...
class DeliverySupabaseUser extends BaseAuthUser {
  DeliverySupabaseUser(this.user);
  User? user;
  ...
  Future? updateEmail(String email) async {
    final response = await SupaFlow.client.auth.updateUser(
      ...
      emailRedirectTo:
          'carteira-contabil:///', // Deep link para abrir o app
    );
...
Stream<BaseAuthUser> deliverySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DeliverySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
